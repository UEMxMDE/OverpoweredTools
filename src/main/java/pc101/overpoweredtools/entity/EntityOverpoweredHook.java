package pc101.overpoweredtools.entity;
...
import pc101.overpoweredtools.objects.items.OverpoweredFishingRod;

import java.util.List;

public class EntityOverpoweredHook extends EntityFishHook
{
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.<Integer>createKey(EntityOverpoweredHook.class, DataSerializers.VARINT);
    ...
    private EntityOverpoweredHook.State currentState = EntityOverpoweredHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    @SideOnly(Side.CLIENT)
    public EntityOverpoweredHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityOverpoweredHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityOverpoweredHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityOverpoweredHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityOverpoweredHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityOverpoweredHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityOverpoweredHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityOverpoweredHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    private boolean shouldStopFishing()
    {
        ...
        boolean flag = itemstack.getItem() instanceof OverpoweredFishingRod;
        boolean flag1 = itemstack1.getItem() instanceof OverpoweredFishingRod;
